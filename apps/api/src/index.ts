...
const sidecarRoot = resolve(fileURLToPath(new URL("../storage/rkf-catalog-sidecar", import.meta.url)));
const catalogStore = RkfCatalogStore.fromBundledArtifact(sidecarRoot, undefined, { postgres: managedStore.catalogPersistence() });
await catalogStore.import({ organizationId: "org-demo", actorId: "system-initialize" });
...
registerAiRoutes(app, managedStore, catalogStore);
