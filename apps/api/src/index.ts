...
app.post("/api/v1/auth/logout", async (request, reply) => { logout(sessionToken(request)); const cookieName = process.env.NODE_ENV === "production" ? "__Host-natacao_session" : "natacao_session"; reply.header("Set-Cookie", `${cookieName}=; HttpOnly; Path=/; Max-Age=0`); return reply.send({ ok: true }); });
...
function requireRoles(request: { headers: Record<string, string | string[] | undefined> }, reply: { code: (status: number) => { send: (value: unknown) => unknown } }, roles: Array<"admin" | "coach" | "athlete">) {
  const user = getSession(sessionToken(request));
...
app.get("/api/v1/dashboard", async (request, reply) => requireRoles(request, reply, ["coach", "admin"]) ? store.dashboard() : undefined);
app.get("/api/v1/athletes", async (request, reply) => requireRoles(request, reply, ["coach", "admin"]) ? ({ data: store.athletes }) : undefined);
app.get("/api/v1/groups", async (request, reply) => requireRoles(request, reply, ["coach", "admin"]) ? ({ data: store.groups }) : undefined);
app.get("/api/v1/pools", async (request, reply) => requireRoles(request, reply, ["coach", "admin"]) ? ({ data: store.pools }) : undefined);
app.get("/api/v1/workouts", async (request, reply) => requireRoles(request, reply, ["coach", "admin"]) ? ({ data: store.workouts, prescriptions: store.prescriptions }) : undefined);
app.get("/api/v1/completed-workouts", async (request, reply) => requireRoles(request, reply, ["coach", "admin"]) ? ({ data: store.completed }) : undefined);
app.get("/api/v1/wellness", async (request, reply) => requireRoles(request, reply, ["coach", "admin"]) ? ({ data: store.wellness }) : undefined);
app.get("/api/v1/integrations", async (request, reply) => requireRoles(request, reply, ["coach", "admin"]) ? ({ data: store.connections, syncJobs: store.syncJobs }) : undefined);
...
app.post("/api/v1/workouts", async (request, reply) => {
  if (!requireRoles(request, reply, ["coach", "admin"])) return;
...
app.post("/api/v1/completed-workouts", async (request, reply) => {
  const user = requireRoles(request, reply, ["coach", "admin", "athlete"]);
  if (!user) return;
