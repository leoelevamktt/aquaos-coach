...
Fontes: ${COACH_BRAIN_MANUAL_SOURCE}, ${COACH_BRAIN_PACKAGE_SOURCE}, Base RKF privada, Catálogo Mestre, banco transacional e histórico de decisões aprovadas.
Você não afirma ser o treinador; reproduz o padrão técnico demonstrado pelas fontes para apoiar a decisão humana.
...
O XLSX é fonte editorial/contratual, não banco de runtime. O runtime usa a projeção estruturada RKF V5.1.
Não invente zonas, fatos ou dados ausentes. Planejado, executado e resposta são registros distintos.
O Planning Engine é normativo para objetivo, zona, volume e fundamento; a IA explica e individualiza sem alterá-los por texto livre.
Sessão-chave exige objetivo, sucesso, alternativa amarela, alternativa vermelha e critério de parada. Toda prescrição exige aprovação do coach.`;
...
function orgRows(store: ManagedStore, kind: ResourceKind, organizationId: string) {
  return store.list(kind).filter((row) => row.organizationId === organizationId);
}

function athleteRows(store: ManagedStore, kind: ResourceKind, organizationId: string, athleteId: string) {
  return recent(orgRows(store, kind, organizationId).filter((row) => row.athleteId === athleteId || (row.targetType === "athlete" && row.targetId === athleteId)));
}
...
function compact(row: ManagedRecord) {
  ...
  return Object.fromEntries(Object.entries(row).flatMap(([key, value]) => {
    if (omit.has(key) || value === null || value === undefined || value === "") return [];
    if (typeof value === "string" && value.length > 1000) return [[key, `${value.slice(0, 1000)}…`]];
    if (Array.isArray(value) && value.length > 30) return [[key, value.slice(0, 30)]];
    return [[key, value]];
  }));
}
...
export function buildAthleteBrainContext(store: ManagedStore, organizationId: string, athleteId: string) {
  ...
  const sections: Array<[ResourceKind, string, number]> = [
    ["athleteProfiles", "Perfil", 20], ["athleteCalibrations", "Calibrações", 20], ["goals", "Metas", 30],
    ["racePlans", "Planos de prova", 30], ["readinessScores", "Readiness", 60], ["athleteResponses", "Respostas/wellness", 60],
    ["loadSnapshots", "Carga", 60], ["loadCalculations", "Cálculos de carga", 60], ["adaptationDecisions", "Adaptações", 60],
    ["prescriptions", "Prescrições/versionamento", 60], ["sessionExecutions", "Execuções", 60], ["results", "Resultados", 60],
    ["performanceBenchmarks", "Benchmarks", 30], ["evolutionAssessments", "Evolução", 30], ["staffAssessments", "Comissão técnica", 40],
    ["sessionContextSnapshots", "Contexto de sessão", 40], ["importedTrainingSessions", "Treinos externos", 30], ["videos", "Vídeos", 30],
  ...
  const audit = store.audit(500).filter((event) => (event.organizationId ?? "org-demo") === organizationId)
    .filter((event) => event.resourceId === athleteId || norm(event.summary).includes(norm(athleteId))).slice(0, 40);
  if (audit.length) parts.push(`## Auditoria relacionada\n${JSON.stringify(audit)}`);
  const text = parts.join("\n\n");
  return text.length <= MAX_CONTEXT_CHARS ? text : `${text.slice(0, MAX_CONTEXT_CHARS)}\n[contexto truncado; fonte integral permanece no banco]`;
}
...
export function buildCoachDecisionEvidence(store: ManagedStore, organizationId: string, athleteId?: string) {
  const prioritize = (rows: ManagedRecord[]) => athleteId
    ...
    : rows;
  const prescriptions = recent(orgRows(store, "prescriptions", organizationId)).filter((row) => row.status === "PUBLISHED" || row.approvedBy || row.publishedSnapshot);
  ...
  return `=== EVIDÊNCIA HISTÓRICA DO PADRÃO DECISÓRIO DO COACH ===\n${JSON.stringify({
    ...
    recentAudit: store.audit(120).filter((event) => (event.organizationId ?? "org-demo") === organizationId).slice(0, 40),
  })}`;
}

function ageOf(row: ManagedRecord) {
  const age = Number(row.age);
  if (Number.isFinite(age) && age >= 8) return Math.floor(age);
  ...
  if (now.getUTCMonth() < birth.getUTCMonth() || (now.getUTCMonth() === birth.getUTCMonth() && now.getUTCDate() < birth.getUTCDate())) value -= 1;
  return value >= 8 ? value : undefined;
}

function eventMeters(value: unknown) {
...
export type DerivedPlanningInputs = { athlete?: PlanningAthleteContext; request?: PlanningRequest; missing: string[]; provenance: string[] };

type PlanningOverrides = Partial<{
  age: number; developmentLevel: "formacao" | "rendimento"; specialty: Specialty; poolLengthM: 25 | 50; eventMeters: number;
  phase: RkfPhaseId; objective: string; primaryZone: ZoneCode; secondaryZone: ZoneCode; targetVolumeM: number;
  rdcMarker: boolean; requiredLegVolumeM: number; skillEmphasis: SkillCode[];
}>;

export function derivePlanningInputs(store: ManagedStore, organizationId: string, athleteId: string, overrides: PlanningOverrides = {}): DerivedPlanningInputs {
  ...
  const latest = prescriptions.find((row) => row.status === "PUBLISHED" || row.publishedSnapshot) ?? prescriptions[0];
  ...
  const meters = overrides.eventMeters ?? (Number(athleteRecord.eventMeters) || eventMeters(goalEvent));
  const specialty = overrides.specialty ?? specialtyOf(athleteRecord, meters);
  if (specialty) provenance.push(overrides.specialty ? "especialidade: override explícito" : athleteRecord.specialty ? "especialidade: cadastro" : "especialidade: derivada da prova");
  const level = overrides.developmentLevel ?? (norm(athleteRecord.developmentLevel) === "formacao" ? "formacao" : norm(athleteRecord.developmentLevel) === "rendimento" ? "rendimento" : age !== undefined ? (age <= 13 ? "formacao" : "rendimento") : undefined);
  if (level) provenance.push(overrides.developmentLevel ? "estágio: override explícito" : athleteRecord.developmentLevel ? "estágio: cadastro" : "estágio: regra por faixa etária");
  const configuredPool = String(settings?.primaryPool ?? "");
  const poolCandidate = overrides.poolLengthM ?? Number(athleteRecord.poolLengthM) || (configuredPool.includes("25") ? 25 : configuredPool.includes("50") ? 50 : 0);
  ...
  const objective = overrides.objective?.trim() || (typeof snapshot.objective === "string" ? snapshot.objective : "") || (typeof goals[0]?.objective === "string" ? String(goals[0].objective) : "") || (goalEvent ? `Preparação para ${String(goalEvent)}` : "");
  if (objective) provenance.push(overrides.objective ? "objetivo: override explícito" : "objetivo: prescrição/meta");
  ...
  const volumeRows = [...athleteRows(store, "sessionExecutions", organizationId, athleteId), ...athleteRows(store, "activities", organizationId, athleteId)];
  const recentVolumes = volumeRows.map((row) => Number(row.prescribedVolumeM ?? row.sessionDistanceM ?? row.executedVolumeM ?? row.distanceMeters)).filter((value) => Number.isFinite(value) && value >= 500).slice(0, 6);
  const weekly = Number(athleteRecord.weeklyDistance);
  const historicalVolume = recentVolumes.length ? Math.round((recentVolumes.reduce((a, b) => a + b, 0) / recentVolumes.length) / 10) * 10 : Number.isFinite(weekly) && weekly > 0 ? Math.round((weekly / 6) / 10) * 10 : undefined;
  ...
  const restrictions = Array.isArray(athleteRecord.restrictions) ? athleteRecord.restrictions.map(String) : typeof athleteRecord.restrictions === "string" ? [athleteRecord.restrictions] : [];
  const recentSessionIds = prescriptions.map((row) => ((row.publishedSnapshot ?? row.prescription ?? {}) as Record<string, unknown>).source)
    .map((source) => typeof source === "object" && source !== null ? String((source as Record<string, unknown>).librarySessionId ?? "") : "").filter(Boolean).slice(0, 12);

  const missing = [
    age === undefined ? "age/birthDate" : "", !level ? "developmentLevel" : "", !specialty ? "specialty/event" : "",
    !poolLengthM ? "poolLengthM" : "", !phase ? "phase" : "", !objective ? "objective/goal" : "", !primaryZone ? "primaryZone" : "", !targetVolumeM ? "targetVolumeM" : "",
  ].filter(Boolean);
  if (missing.length || age === undefined || !level || !specialty || !poolLengthM || !phase || !objective || !primaryZone || !targetVolumeM) return { missing, provenance };

  return {
    athlete: { athleteId, age, developmentLevel: level, specialty, poolLengthM, eventMeters: meters, restrictions },
    request: { phase, objective, primaryZone, secondaryZone: overrides.secondaryZone, targetVolumeM: Math.round(targetVolumeM / 10) * 10, rdcMarker: overrides.rdcMarker ?? false, requiredLegVolumeM: overrides.requiredLegVolumeM, skillEmphasis: overrides.skillEmphasis, readiness, recentSessionIds },
    missing: [], provenance,
  };
}

function getCatalogContext(store: CatalogKnowledgeStore | undefined, organizationId: string, query: string) {
...
async function callCoachLlm(system: string, prompt: string) {
  ...
  try {
    const response = await fetch(`${LLM_BASE_URL}/chat/completions`, {
      ...
      body: JSON.stringify({ model: LLM_MODEL, messages: [{ role: "system", content: system }, { role: "user", content: prompt }], temperature: 0.25, max_tokens: 2200, stream: false }), signal: controller.signal,
    });
...
const requestSchema = z.object({
  age: z.number().int().min(8).max(120).optional(), developmentLevel: z.enum(["formacao", "rendimento"]).optional(),
  specialty: z.enum(["velocidade", "meio_fundo", "fundo"]).optional(), poolLengthM: z.union([z.literal(25), z.literal(50)]).optional(), eventMeters: z.number().positive().optional(),
  phase: z.enum(PHASES).optional(), objective: z.string().trim().min(2).max(400).optional(), primaryZone: z.enum(ZONES).optional(), secondaryZone: z.enum(ZONES).optional(),
  targetVolumeM: z.number().int().min(500).max(30000).multipleOf(10).optional(), rdcMarker: z.boolean().optional(), requiredLegVolumeM: z.number().int().positive().multipleOf(10).optional(),
  skillEmphasis: z.array(z.string().trim().min(1)).max(12).optional(), focus: z.string().trim().max(1000).optional(),
});
...
export function registerCoachBrainRoutes(app: FastifyInstance, store: ManagedStore, catalogStore?: CatalogKnowledgeStore) {
  app.get("/api/v1/ai/coach-brain/status", async (request, reply) => {
    ...
    const packageReady = Boolean(library && library.stats.sessions === 910 && library.stats.blocks === 6226 && library.stats.volumeMismatched === 0);
    return {
      ready: Boolean(packageReady && knowledge && catalog), policyVersion: COACH_BRAIN_POLICY_VERSION,
      sources: {
        implementationManual: { source: COACH_BRAIN_MANUAL_SOURCE, integratedAs: "governança/política decisória", ready: true },
        officialPackageV4: { source: COACH_BRAIN_PACKAGE_SOURCE, integratedAs: "contrato operacional projetado no DB/biblioteca RKF V5.1", ready: packageReady },
      ...
      decisionMemory: {
        approvedPrescriptions: orgRows(store, "prescriptions", user!.organizationId).filter((row) => row.status === "PUBLISHED" || row.approvedBy).length,
  ...
  app.post("/api/v1/ai/athletes/:athleteId/personalized-training", async (request, reply) => {
    ...
    const derived = derivePlanningInputs(store, user!.organizationId, athlete.id, { ...body.data, skillEmphasis: body.data.skillEmphasis as SkillCode[] | undefined });
    if (!derived.athlete || !derived.request) return reply.code(422).send({ error: "Faltam dados confirmados para uma prescrição RKF segura.", missing: derived.missing, provenance: derived.provenance, rule: "A IA não preenche lacunas críticas por palpite." });
    const library = loadRkfLibrary();
    ...
    const query = [body.data.focus, derived.request.objective, derived.request.primaryZone, derived.request.phase, athlete.name].filter(Boolean).join(" ");
    const knowledge = await buildRkfKnowledgeContext(query);
    const catalog = getCatalogContext(catalogStore, user!.organizationId, query);
    const system = [COACH_POLICY, knowledge, catalog].filter(Boolean).join("\n\n");
    const prompt = [
      `ATLETA: ${String(athlete.name ?? athlete.id)} (${athlete.id})`, buildAthleteBrainContext(store, user!.organizationId, athlete.id),
      buildCoachDecisionEvidence(store, user!.organizationId, athlete.id), `=== CANDIDATO NORMATIVO DO PLANNING ENGINE ===\n${JSON.stringify(planning)}`,
      `PROVENIÊNCIA: ${derived.provenance.join("; ")}`, body.data.focus ? `FOCO DO COACH: ${body.data.focus}` : "",
      "Explique a prescrição personalizada sem alterar volume, zonas ou blocos do Planning Engine. Identifique os fatos do atleta que influenciaram a escolha; dê critério de sucesso, alternativa AMARELA, alternativa VERMELHA e critério de parada. Se uma alternativa mudar objetivo/zona/fundamento, declare que exige nova execução do Planning Engine. Termine com o que o treinador precisa confirmar antes de publicar.",
    ].filter(Boolean).join("\n\n");
    let explanation: string | null = null; let llmError: string | undefined;
    try { explanation = await callCoachLlm(system, prompt); } catch (error) { llmError = error instanceof Error ? error.message : "Falha na IA"; }

    return reply.code(planning.status === "PRONTO" ? 200 : 422).send({
      status: planning.status, athlete: { id: athlete.id, name: athlete.name }, personalizedExplanation: explanation, llmError, planning,
      provenance: derived.provenance,
      sources: { manual: COACH_BRAIN_MANUAL_SOURCE, operationalPackage: COACH_BRAIN_PACKAGE_SOURCE, libraryVersion: planning.prescription.versions.seed, libraryPackageHash: library.stats.packageHash, catalogVersion: catalogStore?.status(user!.organizationId)?.version ?? null, knowledgeRag: Boolean(knowledge) },
