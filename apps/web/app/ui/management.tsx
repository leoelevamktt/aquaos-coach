...
export type ManagementKind = "athletes" | "groups" | "workouts" | "seasons" | "meets" | "videos" | "documents" | "staff" | "zones" | "goals" | "activities" | "settings" | "teams" | "macrocycles" | "mesocycles" | "microcycles" | "trainingSessions" | "sessionBlocks" | "sessionExecutions" | "athleteResponses" | "deviceSamples" | "readinessScores" | "performanceBenchmarks" | "evolutionAssessments" | "loadCalculations" | "syncJobs";
...
const modules: { id: Kind; label: string; icon: typeof Users; fields: { key: string; label: string; type?: "date" | "number" | "select"; options?: string[] }[] }[] = [
  ...
  { id: "trainingSessions", label: "Sessões normalizadas", icon: Waves, fields: [{ key: "title", label: "Título" }, { key: "date", label: "Data", type: "date" }, { key: "distanceMeters", label: "Distância (m)", type: "number" }, { key: "zone", label: "Zona" }, { key: "status", label: "Status" }] },
  { id: "sessionBlocks", label: "Blocos de sessão", icon: Layers, fields: [{ key: "name", label: "Nome" }, { key: "sessionId", label: "Sessão" }, { key: "order", label: "Ordem", type: "number" }, { key: "distanceMeters", label: "Distância (m)", type: "number" }, { key: "status", label: "Status" }] },
...
export function ManagementCenter({ onClose, onNotify, initialKind = "athletes", createOnOpen = false }: { onClose: () => void; onNotify: (message: string) => void; initialKind?: ManagementKind; createOnOpen?: boolean }) {
  ...
  return <ModalShell title="Central de gestão" subtitle="Cadastros, arquivos, integrações e auditoria em um só lugar" onClose={onClose} wide className="management-modal">
    <div className="management-shell">
     ...
     {viewing && <div className="nested-panel record-view"><div className="nested-panel-head"><div><h3>{labelOf(viewing)}</h3><p>ID {viewing.id}</p></div><button type="button" className="secondary-button" onClick={() => setViewing(null)}>Fechar</button></div>{typeof viewing.url === "string" && <div className="record-media-preview">{String(viewing.mimeType ?? "").startsWith("video/") ? <video controls preload="metadata" src={mediaUrl(viewing.url)} poster={mediaUrl(typeof viewing.thumbnailUrl === "string" ? viewing.thumbnailUrl : undefined)} /> : String(viewing.mimeType ?? "").startsWith("image/") ? <img src={mediaUrl(viewing.url)} alt={labelOf(viewing)} /> : <div className="record-file-preview"><FileText size={22} /><span><b>Arquivo disponível</b><small>{String(viewing.originalName ?? viewing.filename ?? "Documento")}</small></span></div>}<a className="secondary-button" href={mediaUrl(viewing.url)} target="_blank" rel="noreferrer">Abrir arquivo</a></div>}<div className="record-details">{Object.entries(viewing).filter(([key]) => !["id", "createdAt", "updatedAt", "analysis", "url", "thumbnailUrl"].includes(key)).map(([key, value]) => <div key={key}><span>{key}</span><b>{typeof value === "object" ? JSON.stringify(value) : String(value ?? "-")}</b></div>)}</div></div>}
  </ModalShell>;
}
